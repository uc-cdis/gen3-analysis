...
You can see this in the [dependencies](gen3analysis/routes/injection_dependencies.py) file.

Second, we create or update a resource for new lists that are created. This
is done in the upsert function in the [lists](gen3analysis/routes/lists.py)
route file.
...
to [the context configuration](gen3analysis/routes/route_configurations.py)
...
```bash
docker build -t gen3analysis:latest .
```

To run:

```bash
docker run --name gen3analysis \
--env-file "./.env" \
...
```bash
docker exec -it gen3analysis bash
```

To kill and remove running container:

```bash
docker kill gen3analysis
docker remove gen3analysis
```
