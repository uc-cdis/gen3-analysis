...
async def get_slicing_view(
    ...
    if get_range:
        ...
        with tempfile.NamedTemporaryFile(
            suffix=".bam", delete=False, mode="w+b"
        ) as temp_bam1:
    ...
    # TODO random unique name. Can't use tempfile because the generated file can't be written to and then opened and read
    input_bam = "/Users/paulineribeyre/Projects/gen3-analysis/abc.bam"
    input_bai = f"{input_bam}.bai"
    output_bam = None
    ...
    try:
        ...
        if input_bam and os.path.exists(input_bam):
            # pass
            os.unlink(input_bam)
        if input_bai and os.path.exists(input_bai):
            # pass
            os.unlink(input_bai)
        # if output_bam and os.path.exists(output_bam.name):
        #     os.unlink(output_bam.name)

    async def file_iterator():
        with open(output_bam.name, "rb") as f:
...
def samtools_stream(bam_input_stream, bai_file_path, output_path):
    import subprocess
    import os
    import shutil

    # Example data stream (can be replaced with actual request stream)
    # data_stream = b"alignment_data_in_sam_format\n..."

    # Samtools command to convert SAM to BAM, reading from stdin and writing to stdout
    samtools_cmd = ["samtools", "view", "-b", "-S", f"##idx##{bai_file_path}", "-"]
    print(f'Running command: `{" ".join(samtools_cmd)}`')
    ...
    # Stream the HTTP data into samtools' stdin
    shutil.copyfileobj(bam_input_stream, proc.stdin)
    # Don't manually close stdin here — let communicate handle it

    stdout_data, stderr_data = proc.communicate()
    # print('stdout_data', stdout_data)

    if proc.returncode != 0:
        print(f"samtools error: {stderr_data.decode()}")
        raise Exception("samtools error")
    else:
        with open(output_path, "wb") as f:
            f.write(stdout_data)
    print("samtools_stream done")
